...
export default function ButtonAnswers(props) {
  const { answers, correct, disabledBtn } = useSelector((state) => state.answers); // Puxa informações do reducer answers

  const { timerId, time, timerRunning } = useSelector((state) => state.timer);
  // Puxa informações do reducer timer

  const { setHasClick, difficulty } = props; // Informações que vem de Questions
  ...
  }, [answers, dispatch]); // Monitora alterações em answer

  // Monta um array com respostas corretas e incorretas
  ...
  // Contabiliza pontuação segundo os parametros recebidos no projeto
